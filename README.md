...
작업을 할 때마다 DOM에 새 노드를 추가하고 있는데, 전체 트리를 렌더링하기 전에 브라우저가 이 작업을 방해할 수 있다. 이 경우 사용자는 불완전한 UI를 보게 된다. 그래서 DOM을 변형시키는 부분을 제거해야 한다.
그 대신 fiber의 root를 추적하고 이것을 진행중인 작업 root, wipRoot 라고 부른다. 모든 작업을 마치면(최상위 root까지 도달해서 알고 있음) 전체 fiber 트리를 DOM에 커밋한다.

## Reconciliation
DOM에 내용을 추가하는 것 외에 DOM을 업데이트하거나 삭제할 수도 있다. 이것을 하려면 렌더 함수에서 받은 elements를 DOM에 추가했던 마지막 fiber 트리와 비교해야 한다. 따라서 커밋을 마친 다음 DOM에 마지막으로 커밋한 fiber 트리에 대한 참조를 저장해야 한다. 이것을 currentRoot라고 하자.
또 모든 fiber에 대체 특성을 추가해야 한다. 이것은 이전 커밋 단계에서 DOM에 커밋한 fiber인 오래된 fiber에 대한 링크이다.
